...
def publicationMetadata(queryTimeStr, timeUnit, spAgg, evEnvoDict, eeVars, evEnvoMetaDf, username, password):
...
def openreadyMetadata(queryTimeStr, timeUnit, spAgg, qText, evEnvoDict, eeVars, evEnvoMetaDf, fileSize, username, password):
    # Load environment for jinja2 templates
    file_loader = FileSystemLoader('templates')
    env = Environment(loader=file_loader)
    # 1. Generate mapping file from template
...
def serdifAPI(username, password, eventDF, metadataDF, timeUnit, spAgg, dataFormat):
    # Format columns for serdif openready
    df = eventDF
    print('Converting CSV to python dictionary for the serdif openready ...\n')
    ...
    if dataFormat == 'CSV':
        ...
        datasetVersion = '20211012T120000'
        eeData.to_csv('ee-data-' + datasetVersion + '-QT-' + time_str + '.csv', index=False)
        print('Data set available in your local folder as ee-dataset-QT-' + time_str + '.csv !\n')
    else:
        # If 'CSV' format is not specified, it is assumed that the data is queried for publication
        ...
        # (iii) metadata publication as rdf
        ...
        eeMetadata_publ_rdf = publicationMetadata(
        ...
        # (iv) metadata publication as rdf
        eeMetadata_or_rdf = openreadyMetadata(
            queryTimeStr=time_str,
            username=username,
            password=password,
            evEnvoDict=evEnvoDict,
        ...
        with ZipFile('ee-openready-' + datasetVersion + '-QT-' + time_str + '.zip', 'w') as zip_file:
            ...
            zip_file.writestr('ee-metadata-publication-' + datasetVersion + '-QT-' + time_str + '.ttl', eeMetadata_publ_rdf)
            zip_file.writestr('ee-metadata-openready-' + datasetVersion + '-QT-' + time_str + '.ttl', eeMetadata_or_rdf)

        print(
            '\nOpen-ready data and metadata available in your local folder as ee-openready-' + datasetVersion + '-QT-' + time_str + '.zip !\n')

    return '\nThanks for using the serdif-api to associate environmental data with individual events through location and time\n https://github.com/navarral/serdif-api'
